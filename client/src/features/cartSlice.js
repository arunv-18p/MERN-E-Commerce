...
const getCartItemPrices = (state) => {
  ...
  let subTotalTemp = 0.0;
  let discountTemp = 0.0;
  cartItems.forEach((cartItem) => {
    subTotalTemp += parseFloat(cartItem.price) * cartItem.quantity;
    discountTemp +=
      parseFloat((cartItem.discountPercentage * cartItem.price) / 100) *
      cartItem.quantity;
  });
  const subTotal = subTotalTemp;
  const discount = cartItems.length > 0 ? +discountTemp.toFixed(2) : 0.0;
  const deliveryCharge = cartItems.length > 0 ? 40 : 0.0;
  const totalPriceTemp =
    cartItems.length > 0 ? subTotal - discount + deliveryCharge : 0.0;
...
const cartSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(ADD_ITEM_TO_CART, (state, action) => {
      ...
      const cartItem = {...action.payload, quantity: isItemExist ? isItemExist.quantity + 1 : 1 };
      const cartItems = isItemExist ? state.cartItems.map((item) =>
    ...
    builder.addCase(REMOVE_ITEM_FROM_CART, (state, action) => {
      ...
      const cartItem = { ...action.payload, quantity: isItemExist.quantity - 1 };
      const cartItems = cartItem.quantity < 1
